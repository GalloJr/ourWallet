import { auth, db, provider, signInWithPopup, signOut, onAuthStateChanged, collection, addDoc, query, where, onSnapshot, deleteDoc, doc, updateDoc, getDoc, setDoc } from "./firebase.js";
...
let allCards = [];
let activeWalletId = null; // ID da carteira compartilhada ou individual
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        // Verifica se há uma carteira conjunta vinculada
        await configurarWallet(user.uid);
    } else {
        currentUser = null;
        activeWalletId = null;
        toggleLoading(false);
...
async function configurarWallet(uid) {
    try {
        const userRef = doc(db, "users", uid);
        const userDoc = await getDoc(userRef);

        if (userDoc.exists() && userDoc.data().linkedWalletId) {
            activeWalletId = userDoc.data().linkedWalletId;
            document.getElementById('family-status').classList.remove('hidden');
            document.getElementById('spouse-id').value = activeWalletId;
        } else {
            activeWalletId = uid;
            document.getElementById('family-status').classList.add('hidden');
        }

        // Carrega dados usando o ID ativo
        carregarDados(activeWalletId);
        carregarCartoes(activeWalletId);
    } catch (e) {
...
form.addEventListener('submit', async (e) => {
    ...
    const dateVal = document.getElementById('date').value; // Formato YYYY-MM-DD
    ...
    try {
        ...
        // Se for cartão, atualiza a fatura com o valor TOTAL (reserva de limite)
        ...
        // Redireciona o filtro para o mês da primeira parcela
...
cardForm.addEventListener('submit', async (e) => {
    ...
    try {
        await addDoc(collection(db, "cards"), { uid: activeWalletId, bank, flag, name, last4, bill, createdAt: new Date() });
...
document.getElementById('family-btn').addEventListener('click', abrirModalFamilia);
